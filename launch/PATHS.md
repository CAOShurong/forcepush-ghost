...
**Tip ahead OK:** `main` is honesty-docs ahead of Release tag `v0.1.17` (registry-`npx` ban · `npx github:…` allow · 15s clip · tip-SHA sync). No code change — do **not** cut `v0.1.18` for docs-only. **Do not pin ephemeral tip SHAs** in PATHS (they go stale on the next honesty commit). Pages / `npx github:CAOShurong/forcepush-ghost` follow `main`; Release URL stays the secondary pin.
